...
# ==============================================================================
# SECTION 3: PREDICTION LOGIC (UPGRADED)
# ==============================================================================
def choose_safe_room(recent_100, lose_streak=0, last_win_room=None, user_id=None, issue_id=None):
    try:
        ...
        # === NÂNG CẤP: Chuyên gia bổ sung khi cần phục hồi sau chuỗi thua ===
        def _get_short_term_recency_danger(history, candidates):
            """Chuyên gia cảnh báo về các phòng vừa xuất hiện trong 3 ván gần nhất."""
            if len(history) < 3: return set()
            recent_rooms = set(history[:3])
            return {r for r in candidates if r in recent_rooms}

        def _get_follow_pattern_danger(history, candidates):
            """Chuyên gia cảnh báo về quy luật lặp lại A -> B."""
            if len(history) < 5: return set()
            transitions = Counter(zip(history[1:], history[:-1]))
            if not transitions: return set()
            # Kiểm tra 2 quy luật phổ biến nhất
            for (prev, current), _ in transitions.most_common(2):
                if history[0] == prev and current in candidates:
                    return {current}
            return set()
        ...
        # Kích hoạt chế độ an toàn cao khi đang thua
        if lose_streak >= 1:
            console.print("🔥 [bold yellow]Chuỗi thua >= 1. Kích hoạt chế độ dự đoán an toàn cao.[/bold yellow]")
            all_specialists.extend([_get_short_term_recency_danger, _get_follow_pattern_danger])
    ...
    while True:
        try:
            ...
            if latest_issue_id != session_state["last_known_issue_id"]:
                ...
                # === SỬA ĐỔI: Cược số tiền chính xác, không thêm ngẫu nhiên ===
